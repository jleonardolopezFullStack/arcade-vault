...
// Puro y determinista: mismo seed → mismas filas. Portado 1:1 de data.jsx.
...
export function detailSeed(id: string): number {
  return id.length * 17 + 3;
}

export function hallSeed(id: string): number {
  return id.length * 23 + 7;
}
